...
class AssetsStore extends SqlitePdoDataLayer
{
  ...
  {
    $replace = [':p_cur_type' => $this->quoteText($pCurType),
                ':p_cur_base_dir' => $this->quoteText($pCurBaseDir),
                ':p_cur_to_dir' => $this->quoteText($pCurToDir),
                ':p_cur_path' => $this->quoteText($pCurPath)];
    $query   = <<< EOT
delete
from PLS_CURRENT
